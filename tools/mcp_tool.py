...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        return json.dumps(
            call_mcp_tool(server_name, tool_name, args, timeout=tool_timeout),
            ensure_ascii=False,
        )
...
def call_mcp_tool(
    ...
    *,
    ...
    #
    # Once the cooldown elapses, the breaker transitions to half-open:
    # let the next call through as a probe. Success closes the breaker;
    # failure reopens it via _bump_server_error().
        ...
        if age < _CIRCUIT_BREAKER_COOLDOWN_SEC:
            ...
            return {
                "error": (
                    ...
                    f"tool yet — use alternative approaches or ask the user to "
                    f"check the MCP server."
                )
            }
    ...
    def _call_once_json():
        return json.dumps(_call_once_dict(), ensure_ascii=False)

    try:
        result = _call_once_dict()
        if "error" in result:
            _bump_server_error(server_name)
        else:
            _reset_server_error(server_name)
