...
DEFAULT_GEMINI_ACP_BASE_URL = "acp://gemini"
...
def resolve_provider(
    ...
    *,
    ...
    _PROVIDER_ALIASES = {
        ...
        "gemini-cli-acp": "gemini-acp", "google-gemini-acp": "gemini-acp",
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target in {"copilot-acp", "gemini-acp"}:
...
def _resolve_external_process_command(provider_id: str) -> tuple[str, list[str]]:
    if provider_id == "gemini-acp":
        command = (
            ...
            or os.path.expanduser("~/.npm-global/bin/gemini")
        )
        if not (command and os.path.isabs(command) and os.path.isfile(command) and os.access(command, os.X_OK)):
            command = command or "gemini"
